...
export const checkEnrollment = async (studentId, courseId, courseType) => {
  return await api.get(`/enrollment/check?studentId=${studentId}&courseId=${courseId}&courseType=${courseType}`);
};
...
export const getVideosByCourse = async (courseId, courseType, studentId) => {
  ...
  const res = await api.get(`/videos/course/${courseId}/${courseType}?studentId=${studentId}`);
...
export const searchCourses = (q) => api.get(`/search?q=${q}`);
